...
def copy_to_clipboard_button(text: str, label: str = "📋", tooltip: str = "Copy to clipboard", key: str | None = None) -> None:
    """
    A self-contained HTML button that copies *text* to the clipboard.
    Works on localhost and on HTTPS deployments.
    """
    element_id = key or str(uuid.uuid4()).replace("-", "")
    btn_html = f"""
    <button id="{element_id}" title="{tooltip}" style="background:transparent;border:none;cursor:pointer;font-size:1.1rem;">
        {label}
    </button>
    <script>
    ...
    btn.addEventListener('click', async () => {{
        try {{
            ...
            btn.innerHTML = "✅";
            setTimeout(()=>btn.innerHTML = oldLabel, 1500);
        }} catch(err) {{
    ...
    """
    # height=30 keeps the component compact
    components.html(btn_html, height=30, width=40)
